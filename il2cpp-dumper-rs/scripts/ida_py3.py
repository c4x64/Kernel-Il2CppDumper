...
def set_data_type(addr, type_str):
	...
	if ida_typeinf.parse_decl(pt, None, type_str + ";", 0):
		...
		ida_bytes.create_data(addr, idc.FF_QWORD if idaapi.get_inf_structure().is_64bit() else idc.FF_DWORD, 0, idaapi.BADADDR)
...
if "ScriptString" in data and "ScriptString" in processFields:
	...
	for scriptString in scriptStrings:
		...
		ida_bytes.patch_qword(addr, current_fake_addr) if idaapi.get_inf_structure().is_64bit() else ida_bytes.patch_dword(addr, current_fake_addr)
		
		set_data_type(addr, "const char* const")
